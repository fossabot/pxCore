...
if "%DUKTAPE_SUPPORT%" == "ON" (
  ...
  echo y | "C:\Program Files\PuTTY\pscp.exe" -i pxscene-build.pem.ppk -P 2220 pxscene-setup-exe.zip "ubuntu@96.116.56.119:/var/www/html/edge/windows"
  IF %ERRORLEVEL% NEQ 0 (
    echo "-------------------------- Failed to upload pxscene setup"
  )
  echo y | "C:\Program Files\PuTTY\pscp.exe" -i pxscene-build.pem.ppk -P 2220 build-win32/logs/build_logs.txt "ubuntu@96.116.56.119:/var/www/html/edge/windows"
  IF %ERRORLEVEL% NEQ 0 (
